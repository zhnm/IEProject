...
<html>
    ...
    <body>
        ...
        <div class="grade">
            <h3 class="titlenews">لیست</h3>
            <div class="container">
                <div class="row">
				<form charset=utf-8 method="post" action="<c:url value = "/login/professorlist/courselist/students" context="/MVCSesssion3"/>">
                    <table>
                    <c:forEach items="${list}" var="info">
                        <tr>
                            <c:forEach items="${info}" var="inf" varStatus="myIndex">
                                <c:if test = "${myIndex.getIndex() == 0}"><td><button name="id" type="submit" value="${inf}" />انتخاب</button></td></c:if>
                                <td><c:out value="${inf}"/></td>                               
                            </c:forEach>
                        </tr>
                    </c:forEach>
                    </table>  
